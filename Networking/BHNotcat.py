...
#***WARNING: Trying to "sudo su -" from a CLIENT will prompt the SERVER for the sudoer's password, not the client; ctrl+C escaping that prompt will break EVERYTHING server side; needed to ctrl+Z out, and cannot restart the server presently because it is STILL hanging. Couldn't kill as root either, YIKES; rebooting fixed it though
